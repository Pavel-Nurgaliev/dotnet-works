...
namespace UsersAndRewardsSQL
{
    public class DataBaseDAO
    {
        ...
        public User GetThisUser(int cellUserId)
        {
            ...
            {
                var command = connection.CreateCommand();
                command.CommandText = $"Select top {cellUserId + 1} * From dbo.Users limit" +
                    $"EXCEPT Select top {cellUserId} * From dbo.Users";
        ...
        public Reward GetThisReward(int cellRewardId)
        {
            ...
            {
                var command = connection.CreateCommand();
                command.CommandText = $"Select top {cellRewardId + 1} * " +
                    $"From dbo.Rewards limit EXCEPT " +
                    $"Select top {cellRewardId} * From dbo.Rewards";
